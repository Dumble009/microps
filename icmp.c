...
void icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    {
        errorf("len=%d is shorter than icmp headersize=%d", len, ICMP_HDR_SIZE);
        return;
    }
    ...
    {
    case ICMP_TYPE_ECHO:
        icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, hdr->values, data + sizeof(*hdr), len, iface->unicast, src);
        break;
    default:
        break;
    }
...
int icmp_output(uint8_t type, uint8_t code, uint32_t values, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    struct icmp_hdr *hdr;
    size_t msg_len; // ヘッダとデータを合わせたメッセージ長
    char addr1[IP_ADDR_STR_LEN];
    char addr2[IP_ADDR_STR_LEN];
