...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (!pcb)
    {
        // 使用されていないポートあてのメッセージだった
        mutex_unlock(&mutex);
        return;
    }

    entry = memory_alloc(sizeof(struct udp_queue_entry) + len - sizeof(*hdr));
    entry->foreign.addr = src;
    entry->foreign.port = hdr->src;
    entry->len = len - sizeof(*hdr);
    memcpy(entry->data, data + sizeof(*hdr), entry->len); // NOLINT
...
udp_output(struct ip_endpoint *src, struct ip_endpoint *dst, const uint8_t *data, size_t len)
{
    ...
    hdr->src = src->port;
    hdr->dst = dst->port;
    total = len + sizeof(struct udp_hdr);
...
int udp_open(void)
{
    ...
    {
        mutex_unlock(&mutex);
        errorf("failed to alloc udp_pcb");
        return -1;
    }

    ret_id = udp_pcb_id(pcb);
    mutex_unlock(&mutex);
    return ret_id;
}
...
int udp_close(int id)
{
    ...
    {
        mutex_unlock(&mutex);
        errorf("there is no udp pcb id: %d", id);
        return -1;
    }
    udp_pcb_release(pcb);
    mutex_unlock(&mutex);
...
int udp_bind(int id, struct ip_endpoint *local)
{
    ...
    {
        mutex_unlock(&mutex);
        errorf("end point %s already exists.", ip_endpoint_ntop(local, ep1, sizeof(ep1)));
        return -1;
    }
    pcb->local = *local;
...
udp_recvfrom(int id, uint8_t *buf, size_t size, struct ip_endpoint *foreign)
{
    ...
    memcpy(buf, entry->data, len); // NOLINT
    memory_free(entry);
    return len;
}