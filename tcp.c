...
tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    hdr->dst = hton16(local->port);
    hdr->seq = hton32(seq);
    hdr->ack = hton32(ack);
    hdr->off = (sizeof(struct tcp_hdr) / 4) << 4; // 32ビット (4バイト) 単位の数値に直す。なぜか左に4ビットシフトしないといけないらしい。(Reservedの分らしいけど、なぜ？)
    hdr->flg = flg;
    hdr->wnd = hton16(wnd);
    hdr->sum = 0;
    hdr->up = 0;
    ...
    total = len + sizeof(struct tcp_hdr);
    ...
    if (ip_output(IP_PROTOCOL_TCP, buf, len + sizeof(hdr), local->addr, foreign->addr) == -1)
    {
        return -1;
    }
